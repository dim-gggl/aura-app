...
from artworks.forms import (
    ArtworkForm, ArtistForm, CollectionForm, ExhibitionForm, 
    WishlistItemForm, ArtworkPhotoForm, ArtworkPhotoFormSet
)
from artworks.models import (
    Artwork, Artist, Collection, Exhibition, WishlistItem, 
    ArtworkPhoto, ArtType, Support, Technique, Keyword
)
...
class TestArtworkForm:
    ...
    def test_artwork_form_user_collections_filter(self, user, other_user):
        ...
        other_collection = Collection.objects.create(user=other_user, name="Collection Other")
    ...
    def test_artwork_form_user_exhibitions_filter(self, user, other_user):
        ...
        user_exhibition = Exhibition.objects.create(
            user=user, name="Expo User", location="Lieu", 
            start_date=date.today(), end_date=date.today()
        )
        other_exhibition = Exhibition.objects.create(
            user=other_user, name="Expo Other", location="Lieu", 
            start_date=date.today(), end_date=date.today()
        )
...
class TestWishlistItemForm:
    ...
    def test_wishlist_form_valid_data(self):
        """Test la validation du formulaire WishlistItemForm avec des données valides."""
        form_data = {
            'title': 'Œuvre Souhaitée',
...
class TestArtworkPhotoForm:
    ...
    def test_artwork_photo_form_valid_data(self):
        ...
        form_data = {
            'caption': 'Vue principale de l\'œuvre',
            'is_primary': True
        }
...
class TestFormModelIntegration:
    ...
    def test_artwork_form_complete_workflow(self, user, artist, art_type, support, technique, collection, exhibition):
