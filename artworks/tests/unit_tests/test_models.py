...
from artworks.models import (
    Artwork, ArtworkPhoto, WishlistItem, Artist, Collection, 
    Exhibition, ArtType, Support, Technique, Keyword
)
...
def artist():
    ...
    return Artist.objects.create(
        ...
        death_year=1519,
        nationality="Italien",
        biography="Leonardo da Vinci était un artiste, scientifique et polymathe italien."
    )
...
def collection(user):
    ...
    return Collection.objects.create(
        user=user,
        name="Ma Collection",
        description="Une belle collection d'art"
    )
...
class TestExhibitionModel:
    ...
    def test_exhibition_ordering(self, user):
        ...
        Exhibition.objects.create(
            user=user, name="Expo 1", start_date=date(2023, 1, 1)
        )
        Exhibition.objects.create(
            user=user, name="Expo 2", start_date=date(2023, 6, 1)
        )
        Exhibition.objects.create(
            user=user, name="Expo 3", start_date=date(2023, 3, 1)
        )
...
class TestArtworkModel:
    ...
    def test_artwork_get_dimensions_display(self, artwork, user):
        ...
        assert artwork_full.get_dimensions_display() == "H: 50.00cm x L: 40.00cm x P: 10.00cm"
    ...
    def test_artwork_many_to_many_relationships(self, artwork, collection, exhibition, keyword):
...
class TestArtworkPhotoModel:
    ...
    def test_artwork_photo_ordering(self, artwork):
        ...
        photo1 = ArtworkPhoto.objects.create(artwork=artwork, is_primary=False)
        photo2 = ArtworkPhoto.objects.create(artwork=artwork, is_primary=True)
        photo3 = ArtworkPhoto.objects.create(artwork=artwork, is_primary=False)
...
class TestIntegrationAndEdgeCases:
    ...
    def test_artwork_photo_multiple_primary_different_artworks(self, user):
        """Test que plusieurs photos peuvent être principales si sur des œuvres différentes."""
