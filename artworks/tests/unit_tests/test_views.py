...
from artworks.models import (
    Artwork, Artist, Collection, Exhibition, ArtworkPhoto, 
    WishlistItem, ArtType, Support, Technique, Keyword
)
...
@pytest.fixture
def artwork(user, artist, art_type, support, technique, collection, exhibition, keyword):
...
class TestArtworkViews:
    ...
    def test_artwork_list_view_filters_by_user(self, authenticated_client, user, other_user, artwork):
        ...
        other_artwork = Artwork.objects.create(
            user=other_user,
            title="Œuvre d'un autre utilisateur"
        )
    ...
    def test_artwork_create_view_post_valid(self, authenticated_client, user, artist, art_type):
...
class TestArtworkSpecialViews:
    ...
    def test_artwork_export_html_view(self, authenticated_client, artwork):
        ...
        assert f"attachment; filename='artwork_{artwork.pk}.html'" in response['Content-Disposition']
        assert "La Nuit étoilée" in response.content.decode()
    ...
    def test_artwork_export_pdf_view_success(self, mock_html, authenticated_client, artwork):
        ...
        assert f"attachment; filename='artwork_{artwork.pk}.pdf'" in response['Content-Disposition']
        assert response.content == b'fake_pdf_content'
    
    @patch('weasyprint.HTML', side_effect=ImportError("WeasyPrint not available"))
    def test_artwork_export_pdf_view_no_weasyprint(self, mock_html, authenticated_client, artwork):
    ...
    def test_random_suggestion_filters_by_location_and_date(self, authenticated_client, user):
...
class TestWishlistViews:
    ...
    def test_wishlist_filters_by_user(self, authenticated_client, user, other_user, wishlist_item):
        ...
        other_item = WishlistItem.objects.create(
            user=other_user,
            title="Élément d'un autre utilisateur"
        )
...
class TestArtistViews:
    ...
    def test_artist_create_view_post_valid(self, authenticated_client):
        ...
        # Vérifier la redirection vers le détail
        assert response.url == reverse('artworks:artist_detail', kwargs={'pk': artist.pk})
...
class TestCollectionViews:
    ...
    def test_collection_list_view_with_search(self, authenticated_client, user):
        ...
        collection2 = Collection.objects.create(user=user, name="Collection Classique", description="Art classique")
    ...
    def test_collection_list_filters_by_user(self, authenticated_client, user, other_user, collection):
        """Test que la liste ne montre que les collections de l'utilisateur connecté."""
        other_collection = Collection.objects.create(user=other_user, name="Collection autre utilisateur")
    ...
    def test_collection_create_view_post_valid(self, authenticated_client, user):
        ...
        # Vérifier la redirection vers le détail
        assert response.url == reverse('artworks:collection_detail', kwargs={'pk': collection.pk})
