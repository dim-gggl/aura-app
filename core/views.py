...
def dashboard(request):
    ...
    art_type_stats = (
        ...
        .order_by("-count")[:5]
    )  # Limit to top 5 for dashboard display
...
def search(request):
    ...
    if query:
        ...
        results["artworks"] = (
            ...
            .prefetch_related("artists", "photos")
            .distinct()[:10]
        )  # Limit to 10 results, use distinct() to avoid duplicates from JOINs
    ...
    context = {
        ...
        "total_results": (
            len(results["artworks"]) + len(results["contacts"]) + len(results["notes"])
        ),
    }
...
def custom_404(request, exception):
    ...
    message = f"404 error for path: {request.path} from IP: {client_ip}"
