    ...
    def handle(self, *args, **options):
        ...
        with connection.cursor() as cursor:
            try:
                ...
                """)
                self.stdout.write(
                    self.style.SUCCESS("✅ Permissions granted to 'aura_user' for schema 'aura'.")
                )
                
                # Set search path for the database
                cursor.execute("ALTER DATABASE aura_db SET search_path TO aura, public;")
                self.stdout.write(
                    self.style.SUCCESS("✅ Search path set to 'aura, public' for database 'aura_db'.")
                )
                ...
                self.stdout.write(
                    self.style.ERROR(f"❌ Error creating schema: {e}")
                )
                raise
